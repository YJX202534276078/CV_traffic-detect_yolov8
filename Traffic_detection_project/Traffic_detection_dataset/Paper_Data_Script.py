...
TRAIN_DIR = PROJECT_DIR / 'Traffic_detection_train'
...
if __name__ == '__main__':
    model_path = TRAIN_DIR / 'results/roboflow_721_CoordAttention_ShuffleAttention_yolov8n/weights/best.pt'
    model = YOLO(str(model_path)) # 选择训练好的权重路径
    result = model.val(data=str(TRAIN_DIR / 'Traffic_Detection_Dataset.yaml'),
                       split='val', # split可以选择train、val、test 根据自己的数据集情况来选择.
                       imgsz=640,
                       batch=32,
                       project=str(TRAIN_DIR / 'results/roboflow_721_CoordAttention_ShuffleAttention_yolov8n'),
                       name='paper_data',
                       )
